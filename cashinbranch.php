...
FROM branch b
WHERE b.Status = 1
  AND b.branchId NOT IN ('AGPL000','AGPL999','AGPL216', 'AGPL065', 'AGPL061', 'AGPL030', 'AGPL041') AND b.state NOT IN ('Karnataka')
  $extra
");
...
WHERE a.branchId NOT IN ('AGPL000', 'AGPL999', 'AGPL216', 'AGPL065', 'AGPL061', 'AGPL030', 'AGPL041')
");
...
<div id="wrapper">
	<div class="row content">
		...
		<!-- filters & summary cards commented out as in your snippet -->
	...
	<script>
		$(document).ready(() => {
			...
			stateSelect && stateSelect.addEventListener('change', (state) => {
				...
				for (let index of tdIndex) {
					stateRows.forEach((e) => {
						if (e.className != "theadRow") {
							switch (index) {
								case 2:
									open += parseInt(e.children[index].innerHTML);
									break;
								case 3:
									fund += parseInt(e.children[index].innerHTML);
									break;
								case 6:
									net += parseInt(e.children[index].innerHTML);
									break;
								case 7:
									expense += parseInt(e.children[index].innerHTML);
									break;
								case 8:
									balance += parseInt(e.children[index].innerHTML);
									break;
							}
						}
					});
				}
				document.getElementById('totalOpenBalance') && (document.getElementById('totalOpenBalance').innerHTML = open.toLocaleString('en-IN'));
				document.getElementById('totalFunds') && (document.getElementById('totalFunds').innerHTML = fund.toLocaleString('en-IN'));
				document.getElementById('totalNetA') && (document.getElementById('totalNetA').innerHTML = net.toLocaleString('en-IN'));
				document.getElementById('totalExpense') && (document.getElementById('totalExpense').innerHTML = expense.toLocaleString('en-IN'));
				document.getElementById('totalAvail') && (document.getElementById('totalAvail').innerHTML = balance.toLocaleString('en-IN'));

			}
			// Only call if selector exists
			stateSelect && getStateData(stateSelect.value);
