		...
		async function fetchData(){
			...
			result.forEach((data, i)=>{
				...
				if(data['status'] == "Begin"){
				    ...
				    td4.innerHTML = "<b>Customer Blocked<br><small>(Unblock from Zonal)</small></b>"
				}
			...
			async function fetchData() {
				...
				result.forEach((data, i) => {
					const {
						timeText,
						color
					} = getDiffColor(data['time'], current_date_time);
				...
				//	td4.classList.add("text-center");
					...
					//if (data['quotation'] != '') {
						//td4.innerHTML = "<a href='../xVMEnquiry.php?id=" + data['Id'] + "' class='btn btn-success btn-user' style='margin-right:25px'><i class='fa fa-comments' style='color:#ffa500'></i><b> ENQUIRY</b></a>";
				//	} else {
				//		td4.innerHTML = "<button disabled class='btn btn-success btn-user tooltip' style='margin-right:25px;'>\
...
//					}
					...
					if (data['ornament_docs']) {
						...
						img.style.width = "75px"; // you can adjust the size
					...
					td5.classList.add("text-center");
					if (data['status'] == "Begin") {
						td5.innerHTML = "<a href='updateRegistered.php?id=" + data['Id'] + "' class='btn' type='button'><i class='fa fa-pencil-square-o text-success'style='font-size:16px'></i></a>"
					} else if (data['status'] == "Blocked") {
						td5.innerHTML = "<b>Customer Blocked<br><small>(Unblock from Zonal)</small></b>"
					}

					const td6 = document.createElement("td");
					td6.classList.add("timerDisplay");
					td6.classList.add("text-center");
					td6.textContent = timeText;
			...
			function timerDisplay() {
				...
				table_row.forEach((row, i) => {
					...
					const {
						timeText,
						color
					} = getDiffColor(time, current_date_time);
