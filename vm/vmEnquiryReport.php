...
if ($type == 'VM-HO') {
    ...
    // Get branch list assigned to VM
    $vmBranchListRow = mysqli_fetch_assoc(mysqli_query(
        $con,
        "SELECT branch FROM vmagent WHERE agentId = '$empId'"
    ));

    $extra = "1=1"; // default: all branches (if no mapping)

    if (!empty($vmBranchListRow['branch'])) {
        $branches = array_filter(array_map('trim', explode(',', $vmBranchListRow['branch'])));
        if (!empty($branches)) {
            $quoted = array_map(function ($b) use ($con) {
                return "'" . mysqli_real_escape_string($con, $b) . "'";
            }, $branches);
            $extra = " w.branchId IN (" . implode(',', $quoted) . ") ";
        }
    }
} else {
    include("logout.php");
...
$enquiry_pending = '';
$f = 1;
$not_feasible_not_interested_price_issue = '';
$g = 1;
$wrong_number = '';
$h = 1;
$billed = '';
...
$walkin = mysqli_query($con, "
    ...
    FROM walkin w 
    LEFT JOIN trans t 
        ON (w.mobile = t.phone AND t.date='" . mysqli_real_escape_string($con, $date) . "' AND t.status='Approved')
    LEFT JOIN branch b 
        ON w.branchId = b.branchId
    LEFT JOIN employee e 
        ON b.ezviz_vc = e.empId
    WHERE $extra
      AND (w.date='" . mysqli_real_escape_string($con, $date) . "' OR w.indate='" . mysqli_real_escape_string($con, $date) . "')
      AND w.issue NOT IN ('Rejected')
    ORDER BY w.id ASC
");
        ...
        if ($row['mobile'] == $row['phone'] || $row['issue'] == 'Sold in Attica') {
            ...
            if ($row['status'] == 1) {
                $pending .= "<tr>" .
                    ...
                    "<td>" . $row['remarks'] . "</td>" .
                    "<td>" . $row['time'] . "</td>" .
                    "<td><b><a class='text-success' href='vmEnquiryComment.php?mobile=" . $row['mobile'] . "&id=" . $row['id'] . "'><span class='fa fa-edit'></span></a></b></td>" .
                    "</tr>";
                $a++;
            } else {
