		...
		<div class="col-lg-12">
			...
			<div class="col-sm-2">
				<form action="export.php" enctype="multipart/form-data" method="post">
					<input type="hidden" name="branch" value="<?php if (isset($_GET['branch'])) {
																	echo $_GET['branch'];
																} else {
																	echo "All Branches";
																} ?>">
					<input type="hidden" name="from" value="<?php if (isset($_GET['from'])) {
																echo $_GET['from'];
															} else {
																echo $date;
															} ?>">
					<input type="hidden" name="to" value="<?php if (isset($_GET['to'])) {
																echo $_GET['to'];
															} else {
																echo $date;
															} ?>">
				...
				<div class="panel-body">
					<table id="example1" class="table table-striped table-bordered table-hover">
						...
						<tbody>
							...
							$monthlyTypes = ["Office Rent", "Electricity Bill", "Others", "Internet Bill", "Telephone Bill", "Water Bill", "Salary", "Salary Advance", "Incentives", "Advertisement Expenses", "Office Maintenance", "Legal Expense", "MD Sir Payment", "Stationary Expense", "IT Expense"];
							...
							while ($row = mysqli_fetch_assoc($sql)) {
								...
								$rowStyle = "";
								if (in_array($row['type'], $dailyTypes)) {
									$rowStyle = "style='background-color:#d4edda;'"; // Green
								} elseif (in_array($row['type'], $monthlyTypes)) {
