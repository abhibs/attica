                    ...
                    <div class="table-responsive">
                        <table id="example1" class="table table-hover table-bordered">
                            ...
                            <tbody>
                                ...
                                if ($branchKey !== '') {
                                    $sql = "SELECT id, name, contact_number, location, metal_type, grams, release_amount,
                                        branch, business_type, release_place, release_doc, gold_image,
                                    ...
                                    if ($stmt = mysqli_prepare($con, $sql)) {
                                        ...
                                        while ($row = mysqli_fetch_assoc($res)) {
                                            ...
                                            // raw paths from DB
                                            $releaseDocRaw  = $row['release_doc'] ?? '';
                                            $goldImageRaw   = $row['gold_image'] ?? '';
                                            // $customerIdRaw  = $row['customer_id_doc'] ?? '';
                                            ...
                                            if (!empty($releaseDocRaw)) {
                                                // if DB only stores filename, add folder here:
                                                $releaseDocPath = 'release_docs/' . $releaseDocRaw;
                                                $safePath = htmlspecialchars($releaseDocPath, ENT_QUOTES, 'UTF-8');

                                                if (isImageFile($releaseDocRaw)) {
                                            ...
                                            // Gold Image
                                            echo "<td style='text-align:center;'>";
                                            if (!empty($goldImageRaw)) {
                                                $goldImagePath = 'gold_images/' . $goldImageRaw;
                                                $safePath = htmlspecialchars($goldImagePath, ENT_QUOTES, 'UTF-8');
                                            ...
                                            //     $customerIdPath = 'uploads/customer_id/' . $customerIdRaw;
                                            //     $safePath = htmlspecialchars($customerIdPath, ENT_QUOTES, 'UTF-8');

                                            //     if (isImageFile($customerIdRaw)) {
