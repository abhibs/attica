						...
						<table id="customer-datatable" class="table table-bordered">
							...
							<tbody id="customer-datatable-result">
								<?php
									...
									$query = mysqli_query($con, " SELECT ec.Id, ec.customer, ec.contact, ec.status, ec.branch, ec.remark, ec.block_counter 
									FROM everycustomer ec JOIN branch b ON ec.branch = b.branchId  
									WHERE ec.date = '$date'AND ec.status = 'Blocked' AND b.ezviz_vc = '$employeeId'");
									...
									while($row = mysqli_fetch_assoc($query)){
										...
										$empQuery = mysqli_query($con,"SELECT branchName,branch,employeeId,name,contact FROM users u join branch b on b.branchId=u.branch  and b.ezviz_vc = '$employeeId' join employee e on e.empId=u.employeeId where u.branch='".$row['branch']."'");
