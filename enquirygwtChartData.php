<?php
	...
	if($_POST['type'] == 'Monthly'){
		...
		$sql = "SELECT YEAR(date) AS year,MONTHNAME(date) AS month,SUM(gwt) AS enquiry_count
		FROM walkin
		...
		while($row = mysqli_fetch_assoc($execSQL)){
			$data[$i][0] = $row['month']."\n".$row['year'];
			$data[$i][1] = $row['enquiry_count']/1000;
			$i++;
		}
		...
		$sql = "SELECT DATE(date) as day, SUM(gwt) AS enquiry_count
        FROM walkin
        WHERE date BETWEEN '$from' AND '$to' $state
        GROUP BY DATE(date)
        ORDER BY DATE(date)";
