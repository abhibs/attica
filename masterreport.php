...
$cityBillsQuery = mysqli_fetch_all(mysqli_query(
    $con,
    "SELECT b.city, COUNT(t.id) AS bills, COALESCE(SUM(grossW), 0) AS netW, COALESCE(SUM(netA), 0) AS netA 
    FROM trans t 
...
$stateBillsQuery = mysqli_fetch_all(mysqli_query(
    $con,
    "SELECT b.state, COUNT(t.id) AS bills, COALESCE(SUM(grossW), 0) AS netW, COALESCE(SUM(netA), 0) AS netA 
    FROM trans t 
    RIGHT JOIN branch b ON (t.branchId=b.branchId AND t.date='$date' AND t.status='Approved' AND t.metal='Gold')
...
$karnatakaOtherNetW = round($stateBillData['Karnataka']['netW'] ?? 0);
        ...
        <tr class="subheader">
            <td colspan="4" style='text-align: left;'>Pondicherry </td>
		...
		<td colspan="2" style='background-color:rgb(218, 212, 212);'><?= preg_replace("/\B(?=(\d{2})+(?!\d))/", ",", substr($totalNetAmount, 0, -3)) . ',' . substr($totalNetAmount, -3) ?>
                </td>
            </tr>
    </table>
