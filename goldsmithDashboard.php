...
// If your XAMPP PHP is old, this next line can cause odd parse output.
...
function get_flash(){
  if (!empty($_SESSION['__flash_msg'])) {
    $m = $_SESSION['__flash_msg'];
...
try {
  ...
  // Add unique key on packet_no (idempotent)
  @mysqli_query($con, "ALTER TABLE smelter ADD UNIQUE KEY uniq_packet_no (packet_no)");
} catch (Throwable $e) {
  // ignore if already added or on older MySQL versions
}
    ...
    if ($mode === 'update_after' && $update_id > 0) {
      ...
      if ($srow = mysqli_fetch_assoc($rsS)) {
        $mismatch = mismatch_flag(
          $srow['from_branch'],
          $srow['from_branch'], // branch id mismatch would be captured on create
          $srow['branch_gross'],
          $srow['branch_net'],
          $srow['branch_purity'],
          $srow['before_wt'],
      ...
      /* -----------------------------------------
         If packet already exists, update AFTER
         instead of inserting a duplicate.
      ----------------------------------------- */
      $existingId = 0;
      if ($packet_no !== '') {
        if ($chk = mysqli_prepare($con, "SELECT id FROM smelter WHERE packet_no=? LIMIT 1")) {
          mysqli_stmt_bind_param($chk, "s", $packet_no);
          mysqli_stmt_execute($chk);
          $rsChk = mysqli_stmt_get_result($chk);
          if ($ex = mysqli_fetch_assoc($rsChk)) $existingId = (int)$ex['id'];
          mysqli_stmt_close($chk);
        }
      }
      if ($existingId > 0) {
        // Treat as AFTER update for the existing BEFORE row
        if ($after_purity==='' || $after_wt==='') {
          set_flash('This packet already exists. Please enter both After Purity and After Weight to update.');
          header('Location: '.$_SERVER['PHP_SELF']); exit;
        }

        $after_img = save_upload('after_img', __DIR__.'/goldsmith/After');

        // Recompute mismatch using the existing row snapshot
        $mismatch = 'no';
        if ($qs = mysqli_prepare($con, "SELECT from_branch, before_wt, before_netwt, before_purity, branch_gross, branch_net, branch_purity FROM smelter WHERE id=? LIMIT 1")) {
          mysqli_stmt_bind_param($qs, "i", $existingId);
          mysqli_stmt_execute($qs);
          $rsS = mysqli_stmt_get_result($qs);
          if ($srow = mysqli_fetch_assoc($rsS)) {
            $mismatch = mismatch_flag(
        ...
        set_flash('AFTER updated for existing packet: '.$packet_no);
        header('Location: '.$_SERVER['PHP_SELF']); exit;
      }
      /* -------- end “existing packet” guard; continue with normal INSERT -------- */
...
$qb = "
  ...
  JOIN (
    ...
    WHERE status = 'Approved'
      AND metal  = 'Gold'
      AND TRIM(SUBSTRING_INDEX(CurrentBranch, ',', -1)) = 'HO'
      AND sta = 'Checked'
  ...
  <div class="content">
    <div class="form-card">
      ...
      <div class="form-group">
        <label><strong>Packets to be received</strong></label>
