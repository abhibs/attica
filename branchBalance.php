<?php
	...
	FROM branch b
	WHERE b.status = 1 AND b.branchId != 'AGPL000' AND b.branchId != 'AGPL999' ".$extra);
...
<style>	
	...
	/* Simple styling for custom search */
	#customSearchWrap {
...
<div id="wrapper">
	<div class="row content">
		...
		<div class="col-lg-12">
			<div class="hpanel">
				<div class="panel-body">
					<!-- Custom search box (filters all rows/columns on keyup) -->
							...
							<tbody id="tableBody">
								<?php
									...
									while($row = mysqli_fetch_assoc($sql)){	
										...
										$chequesCount = array_key_exists($row['branchId'], $cheques) ? $cheques[$row['branchId']] : 0;
										...
										echo "<td class='td-align-right'>".$chequesCount."</td>";
										echo "</tr>";
	...
	<script>
		$(document).ready(()=>{
			...
			let dt = $('#exampleBalance').DataTable({
				paging: false,
				"ajax": '',
			...
			// Custom search input: filter across all columns on keyup
				...
				for(let index of tdIndex){
					stateRows.forEach((e)=>{
						if(e.className != "theadRow"){
							switch(index){
								case 2 : open += parseInt(e.children[index].innerHTML) || 0; break;
								case 3 : fund += parseInt(e.children[index].innerHTML) || 0; break;
								case 6 : net += parseInt(e.children[index].innerHTML) || 0; break;
								case 7 : expense += parseInt(e.children[index].innerHTML) || 0; break;
								case 8 : balance += parseInt(e.children[index].innerHTML) || 0; break;
							}
						}
					});
				}
