...
try {
    if ($currentState !== '') {
        $alsoTN = in_array(strtolower(trim($currentState)), ['pondicherry','puducherry'], true);
        $extraStateClause = $alsoTN ? " OR state IN ('Tamil Nadu','Tamilnadu')" : "";
...
// ---------- Handle forwarding + goldtare (single submit) ----------
            ...
            if ($gt['status'] === 'Pending') {
                ...
                if ($newStatus === 'Received') {
                    ...
                    if ($needAppend) {
                        ...
                        mysqli_query($con, "UPDATE goldtare SET status='Received', updated_at=NOW() WHERE id=$id LIMIT 1");
                    }
                            ...
                            <tr>
                              <td colspan="14">
                                ...
                                <button type="submit"
                                        ...
                                        id="btnUpdate"
                                        disabled>
                                  <i class="fa fa-save"></i> Update Forwarding & Save Tare
                                </button>

                                <input type="hidden" id="printed_flag" name="printed_flag" value="0">
                              </td>
                            </tr>
...
(function(){
  ...
  document.addEventListener('change', function(e){
    var first = document.querySelector('.fwd-select');
    if (first && e.target === first) syncTobranchFromFirst();
  ...
  function enableUpdateAfterPrint(){
    if (printedEl) printedEl.value = '1';
    if (btnUpdate) {
  ...
  if (btnPrint && mainForm){
    btnPrint.addEventListener('click', function(){
      var selected = $$all('.row-check:checked');
      if (selected.length === 0){
        alert('Please select at least one row before printing the report.');
        return;
      }
      ...
      // pass first forwarding branch (for PDF header)
  ...
  if (mainForm){
    mainForm.addEventListener('submit', function(e){
      ...
      if (!submitter || submitter.id !== 'btnUpdate') return;

      if (!printedEl || printedEl.value !== '1') {
        e.preventDefault();
        alert('Please click "Print to PDF" first. After printing, you can update forwarding & save tare.');
        return false;
      }

      syncCountsAndBranch();
      var tb = (document.getElementById('tobranch')?.value || '').trim();
      var pc = parseInt(document.getElementById('packetcount')?.value || '0', 10);
      var twStr = (document.getElementById('tareweight')?.value || '').trim();
