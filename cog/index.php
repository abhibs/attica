...
$stmt = $pdo->query("
  ...
  WHERE b.ezviz_vc IN ('1003445','1000211','1003665','1000036','1000423','1002638',
                       ...
                       '1000524','1000735','1003764','1000816')
  GROUP BY e.name
");
$cogMap = [];
...
// ─── WALKIN AND BILLS DATA ─────────────────────────────────────────────────────
$walkinStmt = $pdo->query("
  SELECT e.name, COUNT(w.id) as walkinCount
  FROM branch b
  JOIN walkin w ON w.branchId = b.branchId AND w.date = CURDATE() AND w.issue != 'Rejected'
  JOIN employee e ON e.empId = b.ezviz_vc
  WHERE b.ezviz_vc IS NOT NULL
  GROUP BY e.name
");
$walkinMap = [];
foreach ($walkinStmt->fetchAll() as $row) {
    $walkinMap[strtolower($row['name'])] = $row['walkinCount'];
}

$billStmt = $pdo->query("
  SELECT e.name, COUNT(t.id) as billCount
  FROM branch b
  JOIN trans t ON t.branchId = b.branchId AND t.date = CURDATE() AND t.status = 'Approved'
  JOIN employee e ON e.empId = b.ezviz_vc
  WHERE b.ezviz_vc IS NOT NULL
  GROUP BY e.name
");
...
foreach ($billStmt->fetchAll() as $row) {
    $billMap[strtolower($row['name'])] = $row['billCount'];
}
...
if (isset($_GET['json'])) {
	...
	<?php $zonals = array_unique($zonals);
		foreach ($zonals as $zm): 
              $zCog = $cogMap[strtolower($zm)] ?? 0;
              $walkin = $walkinMap[strtolower($zm)] ?? 0;
              $bills  = $billMap[strtolower($zm)] ?? 0;
            ?>
              <li>
                <?= htmlspecialchars($zm,ENT_QUOTES) ?> (<?= $zCog ?>%)
	      </li>
	      <li>		
		<span style="color:green;">(B-<?= $bills ?>)</span>
                <span style="color:red;">(E-<?= $walkin ?>)</span>
             </li>
            <?php endforeach; ?>
          </ul>
        </div>
