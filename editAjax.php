...
if (isset($_POST["editStock"])) {
    ...
    // 1) Whitelist columns to avoid injection
    $allowedCols = [
    ...
    if ($id <= 0 || !in_array($colName, $allowedCols, true)) {
        echo '0';
        exit;
    }

    // 2) Build query with BACKTICKS around the column name
    $sql = "UPDATE `stock`
            SET `{$colName}` = ?,
                `date` = ?
            WHERE `id` = ?";
    ...
    if (!$stmt) {
        // Debug (optional): echo mysqli_error($con);
        echo '0';
        exit;
    }

    mysqli_stmt_bind_param($stmt, "ssi", $colValue, $date, $id);

    if (mysqli_stmt_execute($stmt)) {
        echo '1';
    } else {
        // Debug (optional): echo mysqli_stmt_error($stmt);
        echo '0';
    }
