<?php
    ...
    if (isset($_GET['mobile']) && isset($_GET['id'])) {
        ...
        $enquiryData = mysqli_fetch_assoc(mysqli_query(
            $con,
            "SELECT w.*, b.branchName
             FROM walkin w, branch b 
             WHERE w.id = $id
               AND w.branchId = b.branchId"
        ));

        $vmData = mysqli_query(
            $con,
            "SELECT idnumber, extra
             FROM everycustomer
             WHERE contact = '$mobile'
               AND date    = '" . $enquiryData['date'] . "'"
        );
    }
                    ...
                    <table class="table table-bordered">
                        <tbody>
                            <tr>
                                ...
                                <!-- Quotation button (UNCHANGED) -->
                                <td class="text-right" style="border-top: hidden; border-right: hidden;">
                                    <?php if ($enquiryData['quotation'] != '') { ?>
                                        <a style="background-color:#123C69; color:#ffffff; padding:5px 15px 5px 15px;"
                                           target="_BLANK"
                                           href="QuotationImage/<?php echo $enquiryData['quotation']; ?>">
                                            Quotation
                                            <span class="fa fa-external-link" style="color:#ffcf40; margin-left: 7px;"></span>
                                        </a>
                                    <?php } else { ?>
                                        No Quotation Given
                                    <?php } ?>
                                ...
                                <!-- MESSAGE button: same design, wired to chat -->
                                <td class="text-right" style="border-top: hidden; border-right: hidden;">
                                    <?php
                                        $branchIdEsc   = htmlspecialchars($enquiryData['branchId'],   ENT_QUOTES, 'UTF-8');
                                        $branchNameEsc = htmlspecialchars($enquiryData['branchName'], ENT_QUOTES, 'UTF-8');
                                    ?>
                                    <a href="javascript:void(0);"
                                       class="btn-msg-branch"
                                       data-branch-id="<?php echo $branchIdEsc; ?>"
                                       data-branch-name="<?php echo $branchNameEsc; ?>"
                                       style="background-color:#123C69; color:#ffffff; padding:5px 15px 5px 15px; display:inline-block;">
                                        Message
                                        <span class="fa fa-external-link" style="color:#ffcf40; margin-left: 7px;"></span>
                                    </a>
                                </td>

                                <!-- Gold Image (UNCHANGED) -->
                                <td class="text-right" style="border-top: hidden; border-right: hidden;">
                                    <?php if ($enquiryData['enquiry_image'] != '') {  ?>
                                        <a style="background-color:#123C69; color:#ffffff; padding:5px 15px 5px 15px;"
                                           target="_BLANK"
                                           href="EnquiryImage/<?php echo $enquiryData['enquiry_image']; ?>">
                                            Gold Image
                                            <span class="fa fa-external-link" style="color:#ffcf40; margin-left: 7px;"></span>
                                        </a>
                                    <?php } else { ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a onclick="copy(this)" id="myInput" class="table-text-decoration">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <select required class="form-control" name="remarks" id="remarks">
