<?php
	...
	$extra = " AND w.branchId IN (SELECT branchId FROM branch WHERE ezviz_vc = '$employeeId')";
	...
	if(isset($_GET['submitlo'])){
		...
		if(isset($_GET['branchId']) && $_GET['branchId'] != ''){
			$walkinSQl = "SELECT w.id, w.name, w.mobile as mobile_temp, concat('XXXXXX', right(w.mobile, 4)) as mobile, w.gold, w.havingG, w.metal, w.gwt, w.nwt, w.ramt, w.remarks, w.issue, w.comment, w.agent_id, w.followUp, w.date, w.time, w.branchId, w.zonal_remarks, t.phone, e.name AS zonal_name
			FROM walkin w 
