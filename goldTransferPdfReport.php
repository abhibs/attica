...
/* CHANGED: column header now Margin % */
...
/* CHANGED: track margin amount separately for overall margin% */
$sn=1; $t_gw=0; $t_nw=0; $t_ga=0; $t_na=0; 
$t_marginAmt=0; // sum of raw margin amounts
...
foreach ($rows as $r) {
    ...
    /* raw margin amount from DB */
    $marginAmt = (float)$r['margin'];

    /* CHANGED: per-row margin% = margin / grossAmount * 100 */
    $marginPerc = 0.0;
    if ($ga > 0) {
        $marginPerc = ($marginAmt / $ga) * 100;
    }

    ensure_space($pdf, 14);

    /* === Transaction row bold + light blue fill === */
    $pdf->SetFont('Times','B',10);
    ...
    /* CHANGED: print margin% instead of margin amount */
    $pdf->Cell($w['margin'],7, num2($marginPerc),   1, 1, "R", true);
    ...
    /* CHANGED: accumulate raw margin amount for total margin% */
    $t_marginAmt += $marginAmt;
...
/* CHANGED: overall margin% = total margin amount / total gross amount * 100 */
$avgMargin = ($t_ga > 0) ? round(($t_marginAmt / $t_ga) * 100, 2) : 0;
...
/* CHANGED: total row also shows margin% */
