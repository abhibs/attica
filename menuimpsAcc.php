...
(function ($) {
    ...
    function refreshZimpsMenuBadge() {
        ...
        // Safety: if badge not present (other menus), do nothing
        if (!$badge.length) return;

        $.getJSON('zonal_imps_chat_unread_total.php', function (res) {

            if (!res || res.success !== true) {
                // Hide badge if API says "no chat" or error
                $badge.hide();
                return;
            }
            ...
            if (isNaN(count) || count <= 0) {
                // No unread → hide badge completely
                $badge.hide();
            } else {
                // Some unread → show total (1+3=4 etc.)
                if (count > 99) {
                    $badge.text('99+');
                } else {
                    $badge.text(count);
    ...
    $(function () {
        ...
        // Poll every 5 seconds for new unread counts
