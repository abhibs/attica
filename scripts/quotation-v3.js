...
class Physical{
	...
	calculateColumn(){
		...
		this.total_MarginA.value = (sum[4] * (3/100));
		this.total_NetA.value = (sum[4] - (+sum[4] * (3/100)));
...
class Release{
	constructor(){
		...
		this.total_NetARelease= document.getElementById('qNetARelease');
		this.total_GrossARelease= document.getElementById('qReleaseAmt');
		this.total_ReleasePayable= document.getElementById('qNetAReleasePay');
		this.givenRateInput = document.getElementById('givenRate');		
		this.rateEvent();
	...
	add_release_data(){
		this.button.addEventListener("click", (event)=>{
			...
			if(releaseA!=='' &&  grossW!=='' && netW!=='' && rate!==''){
				...
				tdRow[2].textContent = (xAmount * 0.91).toFixed(0);
				tdRow[3].textContent = (xAmount * 0.91 * 0.03).toFixed(0);
				tdRow[4].textContent = ((xAmount * 0.91)-(xAmount * 0.91 * 0.03)).toFixed(0);
				tdRow[5].textContent =((xAmount * 0.91 * 0.97)-releaseA).toFixed(0);
				this.total_NetARelease.value = ((xAmount * 0.91)-(xAmount * 0.91 * 0.03)).toFixed(0);
				this.total_GrossARelease.value = this.amount.value;
				this.total_ReleasePayable.value  = ((xAmount * 0.91 * 0.97)-releaseA).toFixed(0);
