		...
		<div class="col-lg-12">
			...
			<div class="hpanel">
				<div class="panel-heading">
					...
					<?php if($billData['status'] == 'Verified'){ ?>
				<a class="btn btn-default a-extra">&#9203; |<span class="timerDisplay1" data-starttime="<?php echo $billData['time']; ?>">00:00</span></a>
				<?php } ?>
					...
					<div class="hpanel">
						<div class="panel-body" style="margin-top:15px">
							<form method="POST" class="form-horizontal" action="xapproveRejectB.php" onkeydown="return event.key != 'Enter';">
								...
								<input type="hidden" name="accTimer" id="accTimer">
								<?php if($billData['type'] == 'Release Gold'){ ?>
...
document.addEventListener("DOMContentLoaded", function () {
	const timerDisplayElements = document.querySelectorAll(".timerDisplay1");
	...
	timerDisplayElements.forEach((td) => {
		const billId = "<?php echo $billData['billId']; ?>"; // unique key per bill
		const localStorageKey = `timerStartTime_${billId}`;

		let startTime;
		const existingStartTime = localStorage.getItem(localStorageKey);

		if (existingStartTime) {
			startTime = new Date(existingStartTime);
		} else {
			startTime = new Date(); // set new start time
			localStorage.setItem(localStorageKey, startTime.toISOString());
		}
...
<script>
document.addEventListener("DOMContentLoaded", function () {
	const form = document.querySelector("form[action='xapproveRejectB.php']");
	const accTimerInput = document.getElementById("accTimer");
	const timerSpan = document.querySelector(".timerDisplay1");

	if (form && accTimerInput && timerSpan) {
		form.addEventListener("submit", function (e) {
			...
			accTimerInput.value = timerValue;

			// Confirm it's set
			console.log("✅ accTimer hidden input set to:", accTimerInput.value);
		});
	} else {
