...
if ($type === 'Branch') {
    // Branch bubble shows branch code
...
} elseif ($type === 'Accounts IMPS') {
    // For Accounts IMPS show the "agent" name from users.agent
    $CHAT_SENDER_LABEL = $username ?: 'Accounts IMPS';
    ...
    if (!empty($username) && isset($con)) {
        ...
        $res  = mysqli_query(
            ...
            "SELECT agent 
             FROM users 
             WHERE employeeId = '$uEsc' 
             LIMIT 1"
        );
        if ($res && mysqli_num_rows($res) > 0) {
            $row = mysqli_fetch_assoc($res);
            if (!empty($row['agent'])) {
                $CHAT_SENDER_LABEL = $row['agent'];
            }
        }
    }

} else {
    // Other center-side users – fallback to employee.name
