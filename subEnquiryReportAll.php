<?php
	...
	if (!empty($branchIds)) {
		...
		$extra = " AND W.branchId IN ($branchIdList)";
	} else {
		$extra = " AND 1=0"; // No access if no branches assigned
	}
	
	// Reset pointer to re-fetch $branchSQL data for $branchData below
	mysqli_data_seek($branchSQL, 0);
	} else {
		include("logout.php");
	}
...
<script>
		document.getElementById('mobileSearch').addEventListener('input', function() {
			...
			if (mobile.length === 10 && /^\d{10}$/.test(mobile)) {
				fetch(`searchEnquiries.php?mobile=${mobile}`)
					.then(response => response.text())
					.then(data => {
						document.querySelector("#example1 tbody").innerHTML = data;
					});
			}
		});
	</script>
