<?php
        ...
        */
        ...
        // Last 3 months: per-day avg(netW) by weekday (today, tomorrow, day+2)
...
<style>
        ...
        /* Row alert colors */
        tr.low-light { background: #fff9cc !important; }   /* light yellow (70–80%) */
        tr.low-dark  { background: #ffe680 !important; }   /* dark yellow  (60–70%) */
        tr.low-red   { background: #ffb3b3 !important; }   /* red          (50–60%) */
        tr.low-bright{ background: #ff6666 !important; }   /* bright red   (<50%)  */
...
<div id="wrapper">
        <div class="row content">
                ...
                <div class="hpanel kpi-legend">
                    ...
                    <div><span class="legend-box" style="display:inline-block;width:14px;height:14px;margin-right:6px;border:1px solid #888;vertical-align:middle;background:#fff9cc;"></span> 70–80% funded </div>
                    <div style="margin-top:6px;"><span class="legend-box" style="display:inline-block;width:14px;height:14px;margin-right:6px;border:1px solid #888;vertical-align:middle;background:#ffe680;"></span> 60–70% funded </div>
                    <div style="margin-top:6px;"><span class="legend-box" style="display:inline-block;width:14px;height:14px;margin-right:6px;border:1px solid #888;vertical-align:middle;background:#ffb3b3;"></span> 50–60% funded </div>
                    <div style="margin-top:6px;"><span class="legend-box" style="display:inline-block;width:14px;height:14px;margin-right:6px;border:1px solid #888;vertical-align:middle;background:#ff6666;"></span> &lt;50% funded </div>
                    </div>
                </div>
                </div>
...
(function(){
  function fillSelect($sel, items){
    $sel.empty();
    (items || []).forEach(it => {
      const opt = new Option(it.text || (it.id+' — '+it.name), it.id);
      $(opt).data('obj', it);
      $sel.append(opt);
    });
  }
  ...
  // Initialize Select2 when modal visible (ensures widths are correct)
  $('#moveCashModal').on('shown.bs.modal', function(){
    $from.select2({width: '100%'}); $to.select2({width: '100%'}); $te.select2({width: '100%'});
  });

  // Build selects on ready
  ...
  // When a FROM branch is selected, compute & show its available balance from the table map
  ...
  $('#fromBranch').on('change', function(){
    ...
    updateFromAvailByBranchId(fromId);
  });

  // Open + prefill TO branch; lock it; let user choose FROM branch
  $(document).on('click', '.btn-move-cash', function(){
    // Read TO branch from the button (this row)
    const toId   = $(this).data('to-id')   || '';
    const toName = $(this).data('to-name') || '';

    // Set & lock the TO branch
    $to.val(toId).trigger('change');
    $to.prop('disabled', true);   // keep TO fixed as requested

    // Reset FROM-related fields
    $from.val(null).trigger('change');  // let user choose FROM
    updateFromAvailByBranchId(null);    // clear/zero shown availability

    // Clear amount & approver
    $('#amount').val('');
    $('#approvedBy').val('');

    // Show the modal
    $('#moveCashModal').modal('show');
  });

  // Submit
  $('#moveCashForm').on('submit', function(e){
    e.preventDefault();

    // NOTE: TO is disabled but still has a value; jQuery .val() reads it fine.
    const payload = {
      ...
      fromAvail:    $('#fromAvail').val()  // will be the FROM branch current availability
    };
    ...
    if (payload.fromBranchId === payload.toBranchId){
      alert('From and To branch cannot be the same.'); return;
    }
...
(function(){
  function fillSelect($sel, items){
    $sel.empty();
    (items || []).forEach(it => {
      const opt = new Option(it.text || it.id, it.id);
      $(opt).data('obj', it);
      $sel.append(opt);
    });
  }
  ...
  const $to   = $('#toBranch');
  const $te   = $('#teId');
  const $appr = $('#approvedBy'); // NEW

  // Initialize Select2 when modal shows
  $('#moveCashModal').on('shown.bs.modal', function(){
    $from.select2({width:'100%'}); 
    $to.select2({width:'100%'}); 
    $te.select2({width:'100%'}); 
    $appr.select2({width:'100%'}); // NEW
  });
  ...
  $(document).ready(function(){
    ...
    fillSelect($to,   window._BRANCH_OPTIONS);
    fillSelect($te,   window._TE_OPTIONS);
    fillSelect($appr, window._APPROVER_OPTIONS); // NEW
  });

  // (… your existing handlers remain …)

  // Submit (no change, now sends approver *id* in approvedBy)
  $('#moveCashForm').on('submit', function(e){
    e.preventDefault();
    const payload = {
      fromBranchId: $('#fromBranch').val(),
      ...
      teId:         $('#teId').val(),
      amount:       $('#amount').val(),
      approvedBy:   $('#approvedBy').val(),   // <- approver id
