...
/* ------------------------------
   Phone search (first screen)
   - On GET with ?search_phone=...
   - If record exists => auto-select latest SI
   - Else => prefill Mobile 1 only
------------------------------- */
$searchedPhone = '';
$prefill_mob1  = '';

if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['search_phone'])) {
    $searchedPhone = preg_replace('/\D+/', '', (string)$_GET['search_phone']); // digits only
    if ($searchedPhone !== '') {
        // Find latest entry for this emp & phone
        $sqlFind = "
            SELECT SI
            FROM ccexcel
            WHERE emp_code = ?
              AND (REPLACE(mob1,' ','') = ? OR REPLACE(mob2,' ','') = ?)
            ORDER BY `date` DESC, `SI` DESC
            LIMIT 1
        ";
        if ($stmt = mysqli_prepare($con, $sqlFind)) {
            mysqli_stmt_bind_param($stmt, 'sss', $empId, $searchedPhone, $searchedPhone);
            mysqli_stmt_execute($stmt);
            $resFind = mysqli_stmt_get_result($stmt);
            $found   = mysqli_fetch_assoc($resFind) ?: null;
            mysqli_stmt_close($stmt);

            if ($found && !empty($found['SI'])) {
                // Behave as if user opened ?si=...
                $_GET['si'] = (string)$found['SI'];
            } else {
                // No existing row – prefill Mobile 1 in new form
                $prefill_mob1 = $searchedPhone;
            }
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Keep your existing meaning for `value`
...
/* ------------------------------
   Decide when to show main form/table
   - Initial plain load => only phone search
------------------------------- */
$showMain = ($_SERVER['REQUEST_METHOD'] === 'POST')
         || isset($_GET['search_phone'])
         || $selected_si !== '';

/* ------------------------------
   Status list (used for select + tabs)
------------------------------- */
$statusList = [
    'Visited Sold',
    'Visited Not Sold',
  ...
  <div class="row content">
    <div class="col-lg-12">
      <div class="hpanel panel-box">
        ...
        <!-- PHONE SEARCH SECTION (always visible) -->
        <div class="panel-body" id="phoneSearchSection">
          <form method="get" class="form-inline" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            <label class="text-success" style="margin-right:6px">Enter Mobile Number:</label>
            <input type="text"
                   class="form-control"
                   name="search_phone"
                   value="<?php echo esc($searchedPhone); ?>"
                   placeholder="Enter mobile number"
                   required>
            <button class="btn btn-success" type="submit"><b>Proceed</b></button>
          </form>
        </div>
            ...
            <div class="row">
              ...
              <div class="col-sm-2">
                ...
                <input type="text"
                       ...
                       name="mob1"
                       id="mob1"
                       value="<?php echo esc($entry['mob1'] ?? ($prefill_mob1 ?: '')); ?>"
                       required>
              </div>
              <div class="col-sm-2">
              ...
              <div class="col-sm-2">
                ...
                <select class="form-control" name="advertisement" id="advertisement">
                  <option value="">-- Select --</option>
                  <?php foreach ($statusList as $_dummy) {} // just to silence unused var warning ?>
                  <?php
      ...
      <div class="hpanel panel-box" style="margin-top:15px">
        ...
        <div class="panel-heading" style="padding-top:10px">
          <ul class="nav nav-tabs" id="statusTabs" style="margin-bottom:10px;">
            <li class="active">
              <a href="#" data-status="">All</a>
            </li>
            <?php foreach ($statusList as $s): ?>
              <li>
                <a href="#" data-status="<?php echo esc($s); ?>">
                  <?php echo esc($s); ?>
                </a>
              </li>
            <?php endforeach; ?>
        ...
        <div class="panel-heading" style="padding-top:10px">
          <form method="get" class="form-inline" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            ...
            <!-- preserve search_phone when filtering date -->
            <?php if ($searchedPhone !== ''): ?>
              <input type="hidden" name="search_phone" value="<?php echo esc($searchedPhone); ?>">
            <?php endif; ?>
...
(function () {
  ...
  var table = null;
  var tableElement = document.getElementById('ccexcelTable');
  if (tableElement) {
    table = new DataTable('#ccexcelTable', {
      order: [[1,'desc'], [0,'desc']],
      pageLength: 25,
      stateSave: true,
      responsive: false,
    ...
    window.addEventListener('resize', function(){ if(table){ table.columns.adjust(); } });
  }

  // STATUS TABS → filter Status column
  ...
  if (statusTabs && table) {
    $('#statusTabs a').on('click', function(e){
      ...
      if (status === '') {
        // show all
        table.column(18).search('').draw(); // COL.STATUS = 18
      } else {
        // exact match on status using regex
        table.column(18).search('^' + status + '$', true, false).draw();
      }
    });
  }
  ...
  // auto-fill online price from ccgold based on metal type
  ...
  function fillFormFromRow(tr){
    ...
    if (formMap.date)             formMap.date.value             = norm(d[COL.DATE]) || todayISO();
    if (formMap.name)             formMap.name.value             = norm(d[COL.NAME]);
    var fullMob1 = getFullPhoneFromCell(tr, COL.MOB1);
    var fullMob2 = getFullPhoneFromCell(tr, COL.MOB2);
    if (formMap.mob1)             formMap.mob1.value             = fullMob1;
    if (formMap.mob2)             formMap.mob2.value             = fullMob2;
    if (formMap.location)         formMap.location.value         = norm(d[COL.LOCATION]);
    if (formMap.district)         formMap.district.value         = norm(d[COL.DISTRICT]);
    if (formMap.business_type)    formMap.business_type.value    = norm(d[COL.BTYPE]) || 'Physical';
    if (formMap.metal_type)       formMap.metal_type.value       = norm(d[COL.METAL]) || '';
    if (formMap.grams)            formMap.grams.value            = norm(d[COL.GRAMS]);
    if (formMap.releasing_amount) formMap.releasing_amount.value = norm(d[COL.REL_AMT]);
    if (formMap.bank_name)        formMap.bank_name.value        = norm(d[COL.BANK]);
    if (formMap.assigned_branch)  formMap.assigned_branch.value  = norm(d[COL.ASSIGNED]);
    if (formMap.online_price)     formMap.online_price.value     = norm(d[COL.OPRICE]);
    if (formMap.price_per_gram)   formMap.price_per_gram.value   = norm(d[COL.PPG]);
    if (formMap.advertisement)    formMap.advertisement.value    = norm(d[COL.AD]);
    ...
    if (formMap.status)           formMap.status.value           = norm(d[COL.STATUS]);
    ...
    if (formMap.submitBtn) formMap.submitBtn.innerText = 'Update Entry';
    setFormActionWithSI(formMap.si.value);

    highlightOnly(tr);
    toggleReleaseFields();
    recalcPricePerGram();
    if (formMap.name) formMap.name.focus();
    var f = document.getElementById('ccForm');
  ...
  function clearForm(){
    if (!formMap.si) return;
    formMap.si.value            = '';
    if (formMap.submitBtn) formMap.submitBtn.innerText = 'Submit Form';

    if (formMap.date)             formMap.date.value             = todayISO();
    if (formMap.name)             formMap.name.value             = '';
    if (formMap.mob1)             formMap.mob1.value             = '';
    if (formMap.mob2)             formMap.mob2.value             = '';
    if (formMap.location)         formMap.location.value         = '';
    if (formMap.district)         formMap.district.value         = '';
    if (formMap.business_type)    formMap.business_type.value    = 'Physical';
    if (formMap.metal_type)       formMap.metal_type.value       = '';
    if (formMap.grams)            formMap.grams.value            = '';
    if (formMap.releasing_amount) formMap.releasing_amount.value = '';
    if (formMap.bank_name)        formMap.bank_name.value        = '';
    if (formMap.assigned_branch)  formMap.assigned_branch.value  = '';
    if (formMap.online_price)     formMap.online_price.value     = '';
    if (formMap.price_per_gram)   formMap.price_per_gram.value   = '';
    if (formMap.advertisement)    formMap.advertisement.value    = '';
    if (formMap.lead)             formMap.lead.value             = 'Cold';
    if (formMap.status)           formMap.status.value           = '';
    if (formMap.remarks_new)      formMap.remarks_new.value      = '';
